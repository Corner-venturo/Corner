    ...
    (set, get) => ({
      ...
      validateLogin: async (
        ...
        try {
          ...
          console.log('[LOGIN DEBUG] Step 1 response status:', validateResponse.status, 'result:', JSON.stringify(validateResult).substring(0, 200))
          ...
          console.log('[LOGIN DEBUG] Step 2: authEmail =', authEmail, 'password length =', password.length)
          const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
            email: authEmail,
            password,
          })
          console.log('[LOGIN DEBUG] Step 2 result:', authError ? 'FAIL: ' + authError.message + ' status=' + authError.status : 'OK')
