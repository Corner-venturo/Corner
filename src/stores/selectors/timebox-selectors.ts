...
export function useWeekStatistics() {
  const scheduledBoxes = useTimeboxStore(state => state.scheduledBoxes);
  const boxes = useTimeboxStore(state => state.boxes);
  ...
  return useMemo(() => {
    ...
    const completionRate = totalBoxes > 0
      ? Math.round((completedBoxes.length / totalBoxes) * 100)
      : 0;
...
export function useWorkoutTrends(weeks: number = 4) {
  const weekRecords = useTimeboxStore(state => state.weekRecords);
...
export function useTodayScheduledBoxes() {
  ...
  return useMemo(() => {
    ...
    return scheduledBoxes.filter(box =>
      box.scheduledDate?.startsWith(today)
    );
  }, [scheduledBoxes]);
}
...
export function useWeekViewBoxes(weekStart: string) {
  ...
  return useMemo(() => {
    ...
    // 過濾該週的 boxes
    const weekBoxes = scheduledBoxes.filter(box => {
