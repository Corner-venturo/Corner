...
export const useTimeboxStore = create<TimeboxState>((set, get) => ({
  ...
  getWeekStatistics: async weekId => {
    const scheduledBoxes = useScheduledBoxesStore
      .getState()
      .items.filter(sb => sb.week_id === weekId)
  ...
  getWorkoutTrends: async (weeks, userId) => {
    const weekRecords = useWeeksStore
      .getState()
      .items.filter(w => w.user_id === userId && !w.archived)
...
export const timeboxHelpers = {
   ...
   */
  createBox: async (
    boxData: Omit<BaseBox, 'id' | 'created_at' | 'updated_at' | 'is_active' | 'sync_status'>
  ) => {
   ...
   * 添加排程箱子
   */
  addScheduledBox: async (
    data: Omit<ScheduledBox, 'id' | 'created_at' | 'updated_at' | 'is_active' | 'sync_status'>
  ) => {
  ...
  copyToNextWeek: async (weekId: string, userId: string) => {
    ...
    const sourceScheduled = useScheduledBoxesStore
      .getState()
      .items.filter(sb => sb.week_id === weekId)
