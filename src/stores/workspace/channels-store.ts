    ...
    (set, get) => ({
      ...
      loadChannels: async workspaceId => {
        ...
        try {
          ...
          const existingChannels = get().channels.filter(
            ch => ch.workspace_id === currentWorkspaceId
          )

          // 2. 從 IndexedDB 載入
          const cachedChannels = ((await localDB.getAll('channels')) as Channel[]).filter(
            ch => ch.workspace_id === currentWorkspaceId
          )

          // 3. 智慧合併 + 去重：優先使用 IndexedDB（更新）、其次 persist（離線備份）
          const otherWorkspaceChannels = get().channels.filter(
            ch => ch.workspace_id !== currentWorkspaceId
          )
          ...
          const mergedChannels = Array.from(channelMap.values()).sort(
          ...
          if (isOnline && process.env.NEXT_PUBLIC_ENABLE_SUPABASE === 'true') {
            setTimeout(async () => {
              try {
                ...
                // 🔥 智慧更新 + 去重：確保不會有重複頻道
                const currentChannels = get().channels.filter(
                  ch => ch.workspace_id === currentWorkspaceId
                )
                const hasChanges =
                  freshChannels.length !== currentChannels.length ||
                  freshChannels.some(
                    fresh =>
      ...
      updateChannel: async (id, updates) => {
        ...
        // 更新本地狀態
        set(state => ({
          channels: state.channels.map(ch => (ch.id === id ? updatedChannel : ch)),
        }))
      ...
      updateChannelOrder: async (channelId, newOrder) => {
        ...
        try {
          ...
          // 更新本地狀態
          set(state => ({
            channels: state.channels.map(ch => (ch.id === channelId ? updatedChannel : ch)),
          }))
            ...
            setTimeout(async () => {
              try {
                ...
                const workspaceGroupIds = new Set(
                  allCachedGroups.filter(g => g.workspace_id === currentWorkspaceId).map(g => g.id)
                )
