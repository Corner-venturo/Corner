...
export function parseAmadeusPNREnhanced(rawPNR: string): EnhancedParsedPNR {
  ...
  const ticketValidations = basic.ticketNumbers && basic.ticketNumbers.length > 0
    ? validateTicketNumbers(basic.ticketNumbers, basic.segments[0]?.airline)
    : []
  
  // 6. 統計警告數量
  const ageWarnings = ageValidations.filter(v => !v.isValid).length + (infantRatioCheck.isValid ? 0 : 1)
  const connectionWarnings = connections.filter(c => !c.isSufficient).length
  const ticketWarnings = ticketValidations.filter(tv => !tv.isValid || tv.warnings.length > 0).length
