...
export class PluginManager {
  ...
  private async getPluginStorage<T>(pluginId: string, key: string): Promise<T | null> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- plugin_storage 表尚未加入型別定義
  ...
  private async savePluginConfig(pluginId: string, config: PluginConfig): Promise<void> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- plugin_configs 表尚未加入型別定義
    await (supabase as any)
      .from('plugin_configs')
      .upsert({
        workspace_id: this.workspaceId,
