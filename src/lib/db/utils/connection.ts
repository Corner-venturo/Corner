...
export async function initDatabase(
  ...
  if (typeof window === 'undefined' || typeof indexedDB === 'undefined') {
    ...
    logger.warn('[LocalDB]', error.message)
    throw error
  }
  ...
  try {
    ...
    logger.error('[LocalDB] 版本檢查失敗:', error)
    // 不阻擋初始化，繼續執行
  }
  ...
  const promise = new Promise<IDBDatabase>((resolve, reject) => {
    ...
    try {
      ...
      request.onerror = event => {
        ...
        logger.error('[LocalDB] request.onerror 觸發:', error, event)
      ...
      request.onupgradeneeded = event => {
        ...
        try {
          ...
          logger.error('[LocalDB] 升級失敗:', error)
          setInitPromise(null) // 清除失敗的 Promise
      ...
      request.onblocked = event => {
        logger.warn('資料庫被其他連線阻擋', event)
      }
    } catch (error) {
      logger.error('[LocalDB] Promise 內部錯誤:', error)
      setInitPromise(null)
      reject(error)
    }
  })
