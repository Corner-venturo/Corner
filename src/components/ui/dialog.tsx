...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onInteractOutside={(e) => {
        // 防止檔案選擇器（系統原生視窗）導致 Dialog 關閉
        // 這是 Radix Dialog 的已知問題
        e.preventDefault()
      }}
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-[10000] rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground cursor-pointer">
        <X className="h-4 w-4" />
