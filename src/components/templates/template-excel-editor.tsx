...
export function TemplateExcelEditor({
  ...
  // 檢查某一列是否在可重複區塊內
  const isRowInRepeatableSection = useCallback(
    (row: number) => {
    ...
    hot.addHook('afterRenderer', (TD: any, row: any, col: any, prop: any, value: unknown) => {
      ...
      if (section) {
        ...
        if (highlightedSection && section.id === highlightedSection.id) {
          ...
          TD.style.borderTop =
            row === section.range.start_row ? '3px solid #f59e0b' : '1px solid #f59e0b'
          TD.style.borderBottom =
            row === section.range.end_row ? '3px solid #f59e0b' : '1px solid #f59e0b'
        } else {
          // 普通可重複區塊樣式
          TD.style.backgroundColor = '#fefce8' // 淡黃色背景
          TD.style.borderLeft = col === 0 ? '2px solid #eab308' : TD.style.borderLeft
          TD.style.borderTop =
            row === section.range.start_row ? '2px solid #eab308' : TD.style.borderTop
          TD.style.borderBottom =
            row === section.range.end_row ? '2px solid #eab308' : TD.style.borderBottom
        }
      }
    })
