...
import {
  type PassengerMatch,
  type SegmentEditData,
  normalizeNameForMatch,
  matchPassengerToMember,
  buildPassengerMatches,
  DEFAULT_SEGMENT_EDIT,
} from './pnr-matching'
...
export function TourPnrToolDialog({
  ...
  const handleParse = useCallback(() => {
    ...
    try {
      ...
      // 自動比對旅客
      setPassengerMatches(buildPassengerMatches(result, members))
  ...
  const handleSegmentEdit = useCallback((index: number, field: keyof SegmentEditData, value: string | boolean) => {
    setSegmentEdits(prev => ({
      ...prev,
      [index]: {
        ...(prev[index] || DEFAULT_SEGMENT_EDIT),
        [field]: value,
      },
    }))
  }, [])
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent level={2} className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
