...
export function BulletinBoard() {
  ...
  const _getBorderColor = (type: string, is_pinned: boolean) => {
    ...
    switch (type) {
      case 'notice':
        return 'border-morandi-red/30'
      case 'event':
        return 'border-morandi-green/30'
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {bulletins.length === 0 ? (
          <div className="morandi-card p-8 text-center">
            ...
            <p className="text-sm text-morandi-secondary/70 mt-1">點擊上方按鈕發布第一則公告</p>
          </div>
        ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-morandi-secondary">
                    <span>{bulletin.author?.display_name || '系統'}</span>
                    <span>{format(new Date(bulletin.created_at), 'yyyy/MM/dd HH:mm')}</span>
      ...
      >
        <DialogContent className="max-w-md morandi-card border-0">
          <DialogHeader>
            <DialogTitle>{editingBulletin ? '編輯公告' : '發布新公告'}</DialogTitle>
          </DialogHeader>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setNewBulletin({ ...newBulletin, type: e.target.value as unknown })
                  }
