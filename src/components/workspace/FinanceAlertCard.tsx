    ...
    <div className="card-morandi p-4 space-y-3">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-semibold text-morandi-primary">{authorName} 分享了財務預警訂單</p>
        ...
        <div className="bg-morandi-red/10 border border-morandi-red/20 rounded-lg p-3">
          ...
          <div className="space-y-2">
            {criticalTours.map(tour => (
              <div key={tour.id} className="bg-white rounded p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-morandi-secondary mt-1">
                      ...
                      | 已收：<span className="text-morandi-red font-medium">NT$ 0</span> ⚠️
                    </p>
                  </div>
                  <Button
                    size="sm"
          ...
          <div className="mt-3 pt-3 border-t border-morandi-red/20">
            <p className="text-sm font-semibold text-morandi-red">
              總計已墊：NT${' '}
              {criticalTours.reduce((sum, t) => sum + t.totalCost, 0).toLocaleString()}
            </p>
          </div>
        </div>
      )}
          ...
          {expanded && (
            <div className="mt-2 space-y-2">
              {partialTours.map(tour => (
                <div key={tour.id} className="bg-morandi-container/20 rounded p-3">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="font-medium text-morandi-primary text-sm">{tour.name}</p>
                      <p className="text-xs text-morandi-secondary mt-1">
                        已付 ${(tour.totalCost / 1000).toFixed(0)}k | 已收 $
                        {(tour.totalRevenue / 1000).toFixed(0)}k | 缺{' '}
      ...
      <div className="pt-3 border-t border-morandi-gold/20">
        <div className="flex items-center justify-between">
          <p className="text-sm text-morandi-secondary">共 {selectedTours.length} 筆訂單</p>
