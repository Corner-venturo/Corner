...
export function usePersonalCanvas(initialCanvasId?: string) {
  ...
  const addNewCanvas = useCallback(async () => {
    ...
    if (personalCanvases.length >= CANVAS_LIMITS.MAX_PERSONAL_CANVASES) {
      alert(`最多只能建立 ${CANVAS_LIMITS.MAX_PERSONAL_CANVASES} 個自訂工作區，請刪除不需要的工作區後再新增。`);
      return;
    }
  ...
  const handleToggleFavorite = useCallback(async (document: RichDocument) => {
    try {
  ...
  const handleUpdateTags = useCallback(async (documentId: string, tags: string[]) => {
    try {
