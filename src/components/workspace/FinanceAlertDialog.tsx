...
export function FinanceAlertDialog({ open, onOpenChange, onShare }: FinanceAlertDialogProps) {
  ...
  const getRiskBadge = (level: string) => {
    switch (level) {
      case 'high':
        return (
          <span className="text-xs px-2 py-0.5 bg-morandi-red/10 text-morandi-red rounded">
            🔴高風險
          </span>
        )
      case 'medium':
        return (
          <span className="text-xs px-2 py-0.5 bg-morandi-gold/10 text-morandi-gold rounded">
            🟡注意
          </span>
        )
      default:
        return (
          <span className="text-xs px-2 py-0.5 bg-morandi-blue/10 text-morandi-blue rounded">
            🟢低風險
          </span>
        )
    }
          ...
          <div className="border-t border-morandi-gold/20 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm text-morandi-secondary">
                已選{' '}
                <span className="text-morandi-primary font-medium">{selectedTours.length}</span>{' '}
                筆， 總缺口{' '}
                <span className="text-morandi-red font-semibold">
                  NT$ {totalGap.toLocaleString()}
                </span>
              </div>
            </div>
