              ...
              {employee.attendance.leave_records.map(record => (
                <div key={record.id} className="bg-white rounded border p-3">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-sm text-morandi-muted">{record.days} 天</span>
                  </div>
                  ...
                  {record.reason && (
                    <div className="text-sm text-morandi-muted mt-1">原因：{record.reason}</div>
                  )}
                </div>
              ))}
            </div>
              ...
              {employee.attendance.overtime_records.map(record => (
                <div key={record.id} className="bg-white rounded border p-3">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-sm text-morandi-muted">{record.hours} 小時</span>
                  </div>
                  <div className="text-sm text-morandi-muted">原因：{record.reason}</div>
                </div>
              ))}
            </div>
          ) : (
