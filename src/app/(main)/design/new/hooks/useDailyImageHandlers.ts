...
export function useDailyImageHandlers({
  ...
  const handleDailyImagePositionSaved = useCallback(
    async (settings: ImageEditorSettings) => {
      ...
      if (currentPage && canvas) {
        ...
        if (coverImageElement && pendingImageUrl) {
          try {
            ...
            let scale = Math.max(targetWidth / originalWidth, targetHeight / originalHeight)
            ...
            ;(newImage as fabric.Image & { name: string; id: string }).id =
              `el-daily-cover-d${currentDayIndex}`
        ...
        Object.assign(newPages[currentPageIndex], {
          dayIndex: pageWithDayIndex.dayIndex ?? currentDayIndex,
        })
