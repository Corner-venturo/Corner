...
import { MapPin, Trash2, Power, Edit2, Plus, Search, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
type SortField = 'name' | 'city' | 'category' | 'duration';
          ...
          <div className="sticky top-0 z-10 bg-gradient-to-r from-morandi-container/40 via-morandi-gold/10 to-morandi-container/40 border-b-2 border-morandi-gold/20 backdrop-blur-sm">
            <div className="flex items-center px-4 py-3 gap-3">
              ...
              <button
                ...
                className="w-48 text-sm font-medium text-morandi-primary flex items-center gap-1 hover:text-morandi-gold transition-colors"
              >
                景點名稱
                {sortField === 'name' && sortDirection === 'asc' && <ArrowUp size={14} />}
                {sortField === 'name' && sortDirection === 'desc' && <ArrowDown size={14} />}
              ...
              <button
                ...
                className="w-32 text-sm font-medium text-morandi-primary flex items-center gap-1 hover:text-morandi-gold transition-colors"
              >
                地點
                {sortField === 'city' && sortDirection === 'asc' && <ArrowUp size={14} />}
                {sortField === 'city' && sortDirection === 'desc' && <ArrowDown size={14} />}
                {sortField !== 'city' && <ArrowUpDown size={14} className="opacity-30" />}
              </button>

              {/* 可排序：類別 */}
              <button
                onClick={() => handleSort('category')}
                className="w-24 text-sm font-medium text-morandi-primary text-center flex items-center justify-center gap-1 hover:text-morandi-gold transition-colors"
              >
                類別
                {sortField === 'category' && sortDirection === 'asc' && <ArrowUp size={14} />}
                {sortField === 'category' && sortDirection === 'desc' && <ArrowDown size={14} />}
                {sortField !== 'category' && <ArrowUpDown size={14} className="opacity-30" />}
              </button>
              ...
              <button
                ...
                className="w-20 text-sm font-medium text-morandi-primary text-center flex items-center justify-center gap-1 hover:text-morandi-gold transition-colors"
              >
                時長
                {sortField === 'duration' && sortDirection === 'asc' && <ArrowUp size={14} />}
                {sortField === 'duration' && sortDirection === 'desc' && <ArrowDown size={14} />}
                {sortField !== 'duration' && <ArrowUpDown size={14} className="opacity-30" />}
              </button>
