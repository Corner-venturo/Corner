  ...
  const initGame = useCallback(async () => {
    ...
    class OfficeScene extends Phaser.Scene {
      ...
      undo() {
        ...
        this.selObjId = null; this.selWall = null; this.selFloor = null
      ...
      showContextMenu(wx: number, wy: number, objId: number) {
        ...
        const menuW = 110, itemH = 28, pad = 4
        ...
        items.forEach((item, i) => {
          ...
          const zone = this.add.zone(menuW / 2, iy + itemH / 2, menuW - 2, itemH)
            .setInteractive({ useHandCursor: true })
          container.add(zone)

          const text = this.add.text(10, iy + 6, item.label, {
            fontSize: '12px', color: '#d0d0d0', fontFamily: 'monospace',
          })
          container.add(text)

          zone.on('pointerover', () => {
            hitBg.clear(); hitBg.fillStyle(0x4ecca3, 0.15); hitBg.fillRect(1, iy, menuW - 2, itemH)
            text.setColor('#4ecca3')
          })
          zone.on('pointerout', () => { hitBg.clear(); text.setColor('#d0d0d0') })
      ...
      hideContextMenu() {
        if (this.contextMenu) { this.contextMenu.destroy(true); this.contextMenu = null }
        ...
        if (action === 'delete') {
          ...
          if (o) {
            const copy: RoomObject = {
              ...o, id: room.nextId++, col: o.col + 0.5, row: o.row + 0.5,
              parentId: undefined, slotIndex: undefined,
            }
      ...
      updatePreview() {
        ...
        if (t === 'floor') {
          const fc = Math.floor(g.col), fr = Math.floor(g.row)
          if (fc < 0 || fr < 0) return
          const p = this.scr(fc + 0.5, fr + 0.5)
          this.previewSprite = this.add.image(p.x, p.y, asset)
            .setOrigin(ANCHORS.floor.x / 64, ANCHORS.floor.y / 64)
            .setScale(this.Z).setDepth(9998).setAlpha(0.45)
        } else if (t === 'wallL') {
          ...
          this.previewSprite = this.add.image(p.x, p.y, asset)
            .setOrigin(0.5).setScale(this.Z).setDepth(9998).setAlpha(0.45)
        }
      }
      ...
      paintCell(col: number, row: number) {
        ...
        if (this.paintType === 'floor') {
          ...
          while (fr >= room.rows) { room.floor.push(new Array(room.cols).fill(null)); room.leftWall.push(null); room.rows++ }
          while (fc >= room.cols) { room.floor.forEach(r => r.push(null)); room.backWall.push(null); room.cols++ }
          room.floor[fr][fc] = this.paintAsset
          const p = this.scr(fc + 0.5, fr + 0.5)
          const spr = this.add.image(p.x, p.y, this.paintAsset)
            .setOrigin(ANCHORS.floor.x / 64, ANCHORS.floor.y / 64)
            .setScale(this.Z).setDepth(1.5) // slightly above existing floor
          ...
          const spr = this.add.image(p.x, p.y, this.paintAsset)
            .setOrigin(ANCHORS.wallL.x / 64, ANCHORS.wallL.y / 64)
            .setScale(this.Z).setDepth(2.5 + fr * 0.01)
          ...
          const spr = this.add.image(p.x, p.y, this.paintAsset)
            .setOrigin(ANCHORS.wallB.x / 64, ANCHORS.wallB.y / 64)
            .setScale(this.Z).setDepth(2.5 + fc * 0.01)
      ...
      create() {
        ...
        this.input.on('pointerdown', (ptr: Phaser.Input.Pointer) => {
          ...
          if (ptr.rightButtonDown()) {
            ...
            if (closest) {
              ...
              this.selWall = null; this.selFloor = null
              this.rebuildAll()
          ...
          if (selectedAssetRef.current) {
            ...
            if (t === 'floor' || t === 'wallL' || t === 'wallB') {
              ...
              if (t === 'floor') {
                while (fr >= room.rows) { room.floor.push(new Array(room.cols).fill(null)); room.leftWall.push(null); room.rows++ }
                while (fc >= room.cols) { room.floor.forEach(row => row.push(null)); room.backWall.push(null); room.cols++ }
            ...
            if (s.col < 0.8 && fr >= 0 && fr < room.rows && room.leftWall[fr]) {
              ...
              this.selObjId = null; this.selFloor = null
              this.rebuildAll(); return
            }
            ...
            if (fc >= 0 && fc < room.cols && fr >= 0 && fr < room.rows && room.floor[fr]?.[fc]) {
              ...
              if (!hasObj) {
                ...
                this.selObjId = null; this.selWall = null
                this.rebuildAll(); return
              }
            }
        ...
        this.input.on('pointermove', (ptr: Phaser.Input.Pointer) => {
          ...
          if (panning && ptr.isDown) {
            this.panX = pox + (ptr.x - psx); this.panY = poy + (ptr.y - psy)
            this.rebuildAll()
            this.updatePreview()
          }
        ...
        this.input.on('pointerup', () => {
          ...
          if (this.isPainting) {
            ...
            this.paintType = null; this.paintAsset = null
            this.paintedSprites.clear()
            saveRoom(room, workspaceId, userId)
        ...
        this.input.keyboard?.on('keydown-DELETE', () => {
          ...
          if (this.selFloor) {
            ...
            this.selFloor = null
            saveRoom(room, workspaceId, userId); this.rebuildAll(); return
          }
          if (this.selWall) {
          ...
          room.objects = room.objects.filter(o => !delIds.has(o.id))
          this.selObjId = null
          saveRoom(room, workspaceId, userId); this.rebuildAll()
        })
        ...
        this.input.keyboard?.on('keydown-F', () => {
          ...
          this.pushHistory()
          const o = room.objects.find(x => x.id === this.selObjId)
          if (o) { o.flip = !o.flip; saveRoom(room, workspaceId, userId); this.rebuildAll() }
        })
      ...
      rebuildAll() {
        ...
        this.selGfx = null; this.selLabelText = null; this.previewSprite = null
        ...
        for (let r = 0; r < room.rows; r++) {
          for (let c = 0; c < room.cols; c++) {
            ...
            try { this.add.image(p.x, p.y, tile).setOrigin(ANCHORS.floor.x / 64, ANCHORS.floor.y / 64).setScale(this.Z).setDepth(1) } catch { /* missing texture */ }
          }
        }
        ...
        room.leftWall.forEach((tile, r) => {
          ...
          try { this.add.image(p.x, p.y, tile).setOrigin(ANCHORS.wallL.x / 64, ANCHORS.wallL.y / 64).setScale(this.Z).setDepth(2 + r * 0.01) } catch { /* missing texture */ }
        })
        ...
        room.backWall.forEach((tile, c) => {
          ...
          try { this.add.image(p.x, p.y, tile).setOrigin(ANCHORS.wallB.x / 64, ANCHORS.wallB.y / 64).setScale(this.Z).setDepth(2 + c * 0.01) } catch { /* missing texture */ }
        })
        ...
        sorted.forEach(o => {
          try {
            ...
            if (editModeRef.current && o.id === this.selObjId) {
              ...
              // Name label
              this.selLabelText = this.add.text(cx, cy + 22 * this.Z, assetLabel(o.asset), {
                fontSize: '10px', color: '#4ecca3', fontFamily: 'monospace',
                backgroundColor: '#0f0f1a', padding: { x: 3, y: 1 },
              }).setOrigin(0.5, 0).setDepth(1000)
            }
          } catch { /* missing texture */ }
        })
      }
    }
