...
import type {
  TourItineraryItem,
  MealSubCategory,
} from '../types/tour-itinerary-item.types'
...
export function useSyncItineraryToCore() {
   ...
   */
  const syncToCore = useCallback(async (params: {
    itinerary_id: string
    tour_id: string | null
    ...
    try {
      ...
      const { data: existing_items, error: fetch_error } = await supabase
        .from('tour_itinerary_items')
        .select('id,category,sub_category,day_number,sort_order,title,quote_status,confirmation_status,leader_status,request_id,quote_item_id')
        .eq('itinerary_id', itinerary_id)

      if (fetch_error) throw fetch_error

      // 2. 分成「純行程項目」和「已有下游資料的項目」
      const items_with_downstream = (existing_items ?? []).filter(item =>
      ...
      for (let day_index = 0; day_index < daily_itinerary.length; day_index++) {
        ...
        if (day.activities) {
          for (const activity of day.activities) {
            ...
            const has_downstream = items_with_downstream.some(
              item => item.day_number === day_number &&
                item.category === ITINERARY_ITEM_CATEGORIES.ACTIVITIES &&
                item.title === activity.title
            )
            if (!has_downstream) {
              new_items.push(activityToItem(activity, day_number, sort, service_date, itinerary_id, tour_id, workspace_id))
            }
            sort++
          }
        }
        ...
        if (day.meals) {
          ...
          for (const [meal_name, sub_cat] of meal_entries) {
            ...
            if (!has_downstream) {
              const item = mealToItem(meal_name, sub_cat, day_number, sort, service_date, itinerary_id, tour_id, workspace_id)
        ...
        if (day.accommodation) {
          ...
          if (!has_downstream) {
            const item = accommodationToItem(day.accommodation, day_number, sort, service_date, itinerary_id, tour_id, workspace_id)
            if (item) new_items.push(item)
          }
          sort++
        }
      ...
      return { success: true, inserted: new_items.length, preserved: items_with_downstream.length }
    } catch (error) {
