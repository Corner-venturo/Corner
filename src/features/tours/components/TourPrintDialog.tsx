...
export function TourPrintDialog({ isOpen, tour, members, onClose }: TourPrintDialogProps) {
  ...
  // 欄位 & 成員選擇
  const toggleColumn = (key: keyof ExportColumnsConfig) => setColumns({ ...columns, [key]: !columns[key] })
  const toggleAllColumns = () => {
    const allSelected = Object.values(columns).every((v) => v)
    setColumns(Object.keys(columns).reduce((acc, key) => ({ ...acc, [key]: !allSelected }), {} as ExportColumnsConfig))
  }
  ...
  const toggleAllMembers = () => {
    setSelectedMembers(selectedMembers.size === members.length ? new Set() : new Set(members.map((m) => m.id)))
  }
  ...
  const handlePrintFlightConfirmation = () => {
    const printMembers = members.filter((m) => selectedMembers.has(m.id))
    openPrintWindow(generateFlightPrintContent({ tour, members: printMembers, pnrData, getAirportName, getAirlineName }))
  }
  ...
  const handleExportExcel = async () => {
    ...
    selectedColumns.forEach((col) => {
      ...
      else if (['total_payable', 'deposit_amount', 'balance'].includes(col)) colWidths.push({ wch: 12 })
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
          ...
          <TabsContent value="members" className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto">
              {(Object.keys(columns) as (keyof ExportColumnsConfig)[]).map((key) => (
                <label key={key} className="flex items-center gap-2 p-2 rounded hover:bg-morandi-bg cursor-pointer">
                  <input type="checkbox" checked={columns[key]} onChange={() => toggleColumn(key)} className="rounded border-morandi-border" />
          ...
          <TabsContent value="flight" className="space-y-4">
            ...
            <MemberSelectionList
              ...
              renderBadge={(member) => member.ticket_number ? <Check size={14} className="text-morandi-green" /> : null}
            />
            <div className="flex justify-end gap-2 pt-4 border-t border-morandi-border">
              <Button variant="outline" onClick={onClose}>
                <X size={16} className="mr-1" />
              ...
              <Button onClick={handlePrintFlightConfirmation} disabled={selectedCount === 0} className="bg-morandi-gold hover:bg-morandi-gold-hover text-white">
                <Printer size={16} className="mr-1" />
                列印 ({selectedCount} 人)
              </Button>
            </div>
          ...
          <TabsContent value="hotel" className="space-y-4">
            ...
            <MemberSelectionList
              ...
              renderBadge={(member) => member.hotel_1_name ? <Check size={14} className="text-morandi-green" /> : null}
            />
            <div className="flex justify-end gap-2 pt-4 border-t border-morandi-border">
              <Button variant="outline" onClick={onClose}>
                <X size={16} className="mr-1" />
              ...
              <Button onClick={handlePrintHotelConfirmation} disabled={selectedCount === 0} className="bg-morandi-gold hover:bg-morandi-gold-hover text-white">
                <Printer size={16} className="mr-1" />
                列印 ({selectedCount} 人)
              </Button>
            </div>
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm">{selectedMembers.size} / {members.length} 人已選</span>
        <Button variant="ghost" size="sm" onClick={toggleAllMembers}>
          {selectedMembers.size === members.length ? TOUR_PRINT_DIALOG_LABELS.取消全選 : TOUR_PRINT_DIALOG_LABELS.全選}
        </Button>
      </div>
