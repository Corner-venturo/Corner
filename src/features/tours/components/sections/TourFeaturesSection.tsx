...
export function TourFeaturesSection({
  data,
  viewMode,
  coverStyle = 'original',
  featuresStyle,
  collageDisplayMode = 'large',
}: TourFeaturesSectionProps) {
  // 決定實際使用的風格：優先使用 featuresStyle，否則根據 coverStyle 推斷
  const effectiveStyle =
    featuresStyle ||
    (coverStyle === 'collage' ? 'collage' : coverStyle === 'art' ? 'art' : 'original')
  ...
  if (effectiveStyle === 'art') {
    return <TourFeaturesSectionArt data={data} viewMode={viewMode} />
  }
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex-shrink-0 aspect-[4/3] cursor-pointer transition-all duration-300 rounded-lg overflow-hidden ${
                                      isActive ? 'opacity-100 scale-100' : 'opacity-70 scale-[0.92]'
                                    }`}
                                    style={{
                                      width: `${cardWidth}%`,
                                      marginRight: `${gap}px`,
                            ...
                            {validImages.map((_, dotIndex) => (
                              <button
                                ...
                                className={`h-2 rounded-full transition-all ${
                                  dotIndex === currentImgIndex ? 'w-4 bg-card' : 'w-2 bg-card/50'
                                }`}
                              />
                            ))}
                          </div>
