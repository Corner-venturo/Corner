...
export function useCanvasElements(options: UseCanvasElementsOptions): UseCanvasElementsReturn {
  ...
  const addImage = useCallback(
    async (url: string, opts?: AddImageOptions) => {
      const canvas = getCanvas()
      if (!canvas) return
  ...
  const addSticker = useCallback(
    (pathData: string, opts?: AddStickerOptions) => {
      const canvas = getCanvas()
      if (!canvas) return
  ...
  const addIcon = useCallback(
    async (iconName: string, opts?: AddIconOptions) => {
      const canvas = getCanvas()
      if (!canvas) return
  ...
  const addIllustration = useCallback(
    async (svgString: string, opts?: AddIllustrationOptions) => {
      const canvas = getCanvas()
      if (!canvas) return
