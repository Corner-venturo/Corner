...
export function useCanvasIO(options: UseCanvasIOOptions): UseCanvasIOReturn {
  ...
  const loadCanvasElements = useCallback(
    async (canvas: fabric.Canvas | null, elements: CanvasElement[]) => {
      if (!canvas) return
  ...
  const exportThumbnail = useCallback(
    (canvas: fabric.Canvas | null, options?: { quality?: number; multiplier?: number }): string => {
      if (!canvas) return ''
