...
export function useReceiptMutations() {
   ...
   */
  const handleLinkPayIntegration = useCallback(async (
    receiptNumber: string,
    item: PaymentItem,
    userId: string,
    tourCode: string
   ...
   * 建立收款單 + 多個收款項目
   */
  const createReceiptWithItems = useCallback(async (
    params: CreateReceiptWithItemsParams
  ): Promise<CreateReceiptWithItemsResult> => {
   ...
   * 更新收款單 + 管理收款項目
   */
  const updateReceiptWithItems = useCallback(async (
    params: UpdateReceiptWithItemsParams
  ): Promise<UpdateReceiptWithItemsResult> => {
