      ...
      <div className="space-y-4">
        {itineraryTitle && (
          <p className="text-sm text-morandi-secondary">
            {SYNC_TO_ITINERARY_DIALOG_LABELS.將同步到行程表}<span className="font-medium text-morandi-primary">{itineraryTitle}</span>
          </p>
        )}
        ...
        {diffs.length === 0 ? (
          ...
          <>
            <p className="text-sm text-morandi-secondary">
              {SYNC_TO_ITINERARY_DIALOG_LABELS.以下餐飲將會更新.replace('{count}', diffs.length.toString())}
            </p>
            <div className="border border-border rounded-lg overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-morandi-container/40">
                  <tr>
                    <th className="text-left py-2 px-3 font-medium text-morandi-primary">{SYNC_TO_ITINERARY_DIALOG_LABELS.天數}</th>
                    <th className="text-left py-2 px-3 font-medium text-morandi-primary">{SYNC_TO_ITINERARY_DIALOG_LABELS.餐別}</th>
                    <th className="text-left py-2 px-3 font-medium text-morandi-primary">{SYNC_TO_ITINERARY_DIALOG_LABELS.原本}</th>
                    <th className="text-center py-2 px-3 font-medium text-morandi-primary w-10"></th>
                    <th className="text-left py-2 px-3 font-medium text-morandi-primary">{SYNC_TO_ITINERARY_DIALOG_LABELS.更新後}</th>
                  </tr>
                </thead>
                <tbody>
                  {diffs.map((diff, index) => (
                    <tr key={index} className="border-t border-border/50">
                      ...
                      <td className="py-2 px-3 text-morandi-secondary">
                        {diff.oldValue || <span className="text-morandi-muted">{SYNC_TO_ITINERARY_DIALOG_LABELS.空}</span>}
                      </td>
                      ...
                      <td className="py-2 px-3">
                        <span className={diff.newValue === SYNC_TO_ITINERARY_DIALOG_LABELS.自理 ? 'text-status-warning font-medium' : 'text-morandi-green font-medium'}>
                          {diff.newValue || <span className="text-morandi-muted">{SYNC_TO_ITINERARY_DIALOG_LABELS.空}</span>}
                        </span>
                      </td>
                    </tr>
                  ))}
