      ...
      <td className="py-3 px-4 text-sm text-morandi-secondary text-center table-divider">
        {item.name === '成人機票' ? (
          <input
            ...
            onChange={(e) => handleUpdateItem(categoryId, item.id, 'adult_price', Number(e.target.value) || 0)}
            className="w-full px-1 py-1 text-sm text-center bg-transparent border-0 focus:outline-none focus:bg-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            placeholder="成人票價"
          />
        ) : item.name === '小孩機票' ? (
          <input
            ...
            onChange={(e) => handleUpdateItem(categoryId, item.id, 'child_price', Number(e.target.value) || 0)}
            className="w-full px-1 py-1 text-sm text-center bg-transparent border-0 focus:outline-none focus:bg-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            placeholder="小孩票價"
          />
        ) : item.name === '嬰兒機票' ? (
          <input
            ...
            onChange={(e) => handleUpdateItem(categoryId, item.id, 'infant_price', Number(e.target.value) || 0)}
            className="w-full px-1 py-1 text-sm text-center bg-transparent border-0 focus:outline-none focus:bg-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            placeholder="嬰兒票價"
          />
        ) : (
