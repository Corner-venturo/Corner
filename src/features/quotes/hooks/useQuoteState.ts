...
export const useQuoteState = () => {
  ...
  const getCountryAndCityFromLocation = useCallback((location: string) => {
  ...
  // 多身份人數統計
  const [participantCounts, setParticipantCounts] = useState<ParticipantCounts>(quote?.participant_counts || {
    adult: 1,
    child_with_bed: 0,
    child_no_bed: 0,
    single_room: 0,
  ...
  // 多身份售價
  const [sellingPrices, setSellingPrices] = useState<SellingPrices>(quote?.selling_prices || {
    adult: 0,
    child_with_bed: 0,
    child_no_bed: 0,
    single_room: 0,
