...
export const useCategoryOperations = ({
  ...
  // 更新所有領隊分攤項目
  const updateGuideItems = useCallback((categories: CostCategory[]) => {
    // 重新計算領隊費用
  ...
  // 新增導遊項目
  const handleAddGuideRow = useCallback((category_id: string) => {
    const totalGuideCost = calculateGuideWithCategories(categories);
  ...
  // 新增成人機票
  const handleAddAdultTicket = useCallback((category_id: string) => {
    const newItem: CostItem = {
      id: Date.now().toString(),
      name: '成人機票',
      quantity: 0,
  ...
  // 新增小孩機票
  const handleAddChildTicket = useCallback((category_id: string) => {
    const newItem: CostItem = {
      id: Date.now().toString(),
      name: '小孩機票',
      quantity: 0,
  ...
  // 新增嬰兒機票
  const handleAddInfantTicket = useCallback((category_id: string) => {
    const newItem: CostItem = {
      id: Date.now().toString(),
      name: '嬰兒機票',
      quantity: 0,
  ...
  const handleUpdateItem = useCallback((category_id: string, itemId: string, field: keyof CostItem, value: unknown) => {
    setCategories(prev => {
