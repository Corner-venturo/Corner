...
import { generateId, createEmptyAttraction, createEmptyDay } from '@/types/timeline-itinerary.types'
...
export function useItineraryDialogData(
  pkg: ProposalPackage | null,
  onSave?: (data: TimelineItineraryData) => Promise<void>
) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const inputRef = useRef<HTMLInputElement>(null)
  const textareaRef = useRef<HTMLTextAreaElement>(null)
  const [uploadTarget, setUploadTarget] = useState<{ dayId: string; attractionId: string } | null>(
    null
  )
  ...
  const updateDayTitle = useCallback(
    (title: string) => {
      setData(prev => ({
        ...prev,
        days: prev.days.map((day, idx) => (idx === activeDayIndex ? { ...day, title } : day)),
      }))
    },
    [activeDayIndex]
  )

  const updateDayAccommodation = useCallback(
  ...
  const insertSymbolToTitle = useCallback(
    (symbol: string) => {
  ...
  const handleHotelSelect = useCallback(
    (selectedHotels: LuxuryHotel[]) => {
      if (selectedHotels.length === 0) return
      const hotel = selectedHotels[0]
      const displayName = hotel.city_name ? `${hotel.name}（${hotel.city_name}）` : hotel.name
  ...
  const handleImageUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const currentAttraction = currentDay?.attractions.find(
        a => a.id === uploadTarget.attractionId
      )
  ...
  const removeImage = useCallback(
    (attractionIndex: number, imageId: string) => {
      setData(prev => ({
        ...prev,
        days: prev.days.map((day, idx) =>
          idx === activeDayIndex
