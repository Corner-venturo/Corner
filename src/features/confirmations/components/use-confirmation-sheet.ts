...
import { syncConfirmationCreateToCore } from '@/features/tour-confirmation/services/confirmationCoreTableSync'
...
export function useConfirmationSheet({
  ...
  const insertConfirmationItems = useCallback(
    async (sheetId: string) => {
      ...
      const existingKeys = new Set(
        (existingItems || []).map(
          item => `${item.category}-${item.supplier_name}-${item.title}-${item.service_date || ''}`
        )
      )
      ...
      // 2. 從報價單項目產生
      const quoteItemsFiltered = quoteItems.filter(item => {
      ...
      quoteItemsFiltered.forEach((item, index) => {
        ...
        const useGroupSize =
          (item.category === 'meal' || item.category === 'activity') && quoteGroupSize
