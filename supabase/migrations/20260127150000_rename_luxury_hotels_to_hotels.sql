...
-- 重命名表
ALTER TABLE IF EXISTS public.luxury_hotels RENAME TO hotels;

-- 重命名索引（PostgreSQL 會自動處理大部分情況，但為了命名一致性還是改一下）
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_country') THEN
    ALTER INDEX idx_luxury_hotels_country RENAME TO idx_hotels_country;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_city') THEN
    ALTER INDEX idx_luxury_hotels_city RENAME TO idx_hotels_city;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_brand') THEN
    ALTER INDEX idx_luxury_hotels_brand RENAME TO idx_hotels_brand;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_active') THEN
    ALTER INDEX idx_luxury_hotels_active RENAME TO idx_hotels_active;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_star') THEN
    ALTER INDEX idx_luxury_hotels_star RENAME TO idx_hotels_star;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_luxury_hotels_class') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_luxury_hotels_updated_at') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_luxury_hotels_updated_at') THEN
