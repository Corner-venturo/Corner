-- 為現有團補建提案和套件
-- 讓所有團都有統一的 Proposal → Package → Tour 結構
...
SELECT
  ...
  CASE
    WHEN t.departure_date IS NOT NULL AND t.return_date IS NOT NULL
    THEN (t.return_date - t.departure_date + 1)::integer
    ELSE NULL
  END as days,
  CASE
    WHEN t.departure_date IS NOT NULL AND t.return_date IS NOT NULL
    THEN (t.return_date - t.departure_date)::integer
    ELSE NULL
  END as nights,
  t.max_participants as group_size,
  true as is_selected,
...
WHERE pp.proposal_id = p.id
  ...
  AND p.selected_package_id IS NULL;
...
WHERE q.tour_id = t.id
  AND t.proposal_package_id IS NOT NULL
  AND q.proposal_package_id IS NULL;
...
SET itinerary_id = (
  ...
  WHERE i.proposal_package_id = pp.id
  LIMIT 1
)
