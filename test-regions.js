...
const supabase = createClient(
  ...
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ||
    'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBmcXZkYWN4b3dwZ2ZhbXV2bnNuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Mjk4MjgxMzksImV4cCI6MjA0NTQwNDEzOX0.fhXYmzbMVkYmjGaARmzwAyYQZ7z_Epm54TJMl8WYpTY'
)
...
async function testRegionsData() {
  ...
  // 測試 regions
  const { data: regions, error: regionsError } = await supabase.from('regions').select('*').limit(5)
  ...
  // 測試 cities
  const { data: cities, error: citiesError } = await supabase.from('cities').select('*').limit(5)
