...
async function checkMembers() {
  ...
  const {
    ...
    count,
    error,
  } = await client.from('members').select('*', { count: 'exact' }).limit(10)
  ...
  if (members && members.length > 0) {
    ...
    members.slice(0, 5).forEach(m => {
      console.log(
        ...
        (m.id || '').slice(0, 8),
        '| order_id:',
        (m.order_id || 'null').slice(0, 8),
        '| name:',
        m.chinese_name || m.name || 'N/A'
      )
    })
  } else {
    console.log('(空表)')
  ...
  const {
    ...
    count: omCount,
    error: omError,
  } = await client.from('order_members').select('*', { count: 'exact' }).limit(10)
