...
async function checkAllQuotes() {
  try {
      ...
      if (quote.quote_type === 'quick') {
        ...
        if (itemCount > 0) {
          quote.quick_quote_items.forEach((item, i) => {
            console.log(`      ${i + 1}. ${item.description} - 數量:${item.quantity} 單價:${item.unit_price} 金額:${item.amount}`);
          });
        }
      }
      console.log('');
    ...
    if (quoteItems.length === 0) {
      ...
      Object.entries(itemsByQuote).forEach(([code, data]) => {
        ...
        data.items.forEach((item, i) => {
          console.log(`   ${i + 1}. ${item.description || item.name} - 數量:${item.quantity} 單價:${item.unit_price} 總價:${item.total_price}`);
        });
        console.log('');
      });
    }
    ...
    if (quickQuotesWithItems.length > 0) {
      console.log(`   ❌ 發現 ${quickQuotesWithItems.length} 筆快速報價單仍有 quote_items（應該刪除）`);
      hasIssues = true;
    } else {
