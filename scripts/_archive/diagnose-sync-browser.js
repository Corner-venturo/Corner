...
;(async function diagnoseSyncStatus() {
  ...
  for (const tableName of tables) {
    try {
      ...
      // 檢查 TBC 項目
      const tbcItems = allItems.filter(
        item => item.code && typeof item.code === 'string' && item.code.startsWith('TBC-')
      )
  ...
  console.log('📊 診斷結果：\n')
  console.log(
    `%c總待同步項目數：${totalPending}`,
    'font-size: 16px; font-weight: bold; color: ' + (totalPending > 0 ? '#ff6b6b' : '#51cf66')
  )
  console.log('')
