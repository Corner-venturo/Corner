...
async function migrateFromRequests(): Promise<TourItineraryItem[]> {
  ...
  const { data: requests, error } = await supabase.from('tour_requests').select('*')

  if (error) {
    console.error('Error fetching tour_requests:', error)
    return []
  }
...
async function migrateFromConfirmations(): Promise<TourItineraryItem[]> {
  ...
  if (error) {
    console.error('Error fetching confirmation items:', error)
    return []
  }
...
async function main() {
  ...
  for (let i = 0; i < allItems.length; i += BATCH_SIZE) {
    ...
    const { error } = await supabase.from('tour_itinerary_items').insert(batch)
