  ...
  try {
    ...
    if (empError) {
      ...
      employees.forEach(emp => {
        ...
        console.log(
          `      格式: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(emp.id) ? 'UUID' : emp.id.match(/^\d+$/) ? '純數字/時間戳記' : 'TEXT'}`
        )
    ...
    if (workspaces && workspaces.length > 0) {
      ...
      console.log(
        `   格式: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(workspaces[0].id) ? 'UUID' : 'TEXT'}`
      )
    }

    // 3. 檢查 channels 表
    console.log('\n📊 3. CHANNELS 表：')
    const { data: channels } = await supabase.from('channels').select('id, name, tour_id').limit(1)

    if (channels && channels.length > 0) {
      console.log(`   ID: ${channels[0].id}`)
      console.log(
        `   格式: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(channels[0].id) ? 'UUID' : 'TEXT'}`
      )
      if (channels[0].tour_id) {
        console.log(`   tour_id: ${channels[0].tour_id}`)
        console.log(
          `   tour_id 格式: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(channels[0].tour_id) ? 'UUID' : 'TEXT'}`
        )
      }
    }
    ...
    // 4. 檢查 tours 表
    console.log('\n📊 4. TOURS 表：')
    const { data: tours } = await supabase.from('tours').select('id, code, name').limit(1)

    if (tours && tours.length > 0) {
      console.log(`   ID: ${tours[0].id}`)
      console.log(
        `   格式: ${/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(tours[0].id) ? 'UUID' : 'TEXT'}`
      )
    ...
    console.log('當前狀況：')
    console.log(
      `   - employees.id: ${employees?.[0]?.id ? (/^[0-9a-f-]{36}$/i.test(employees[0].id) ? 'UUID ✅' : employees[0].id.match(/^\d+$/) ? '時間戳記 ⚠️' : 'TEXT ⚠️') : '未知'}`
    )
    console.log(`   - workspaces.id: ${workspaces?.[0]?.id ? 'UUID ✅' : '未知'}`)
    console.log(`   - channels.id: ${channels?.[0]?.id ? 'UUID ✅' : '未知'}`)
    console.log(
      `   - tours.id: ${tours?.[0]?.id ? (/^[0-9a-f-]{36}$/i.test(tours[0].id) ? 'UUID ✅' : 'TEXT ✅') : '未知'}`
    )
