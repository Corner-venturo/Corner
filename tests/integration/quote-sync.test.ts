    ...
    if (cat.id === 'meals') {
      const itemsWithPrices = newMealsItems.map(newItem => {
        ...
        if (match) {
          ...
          if (existing)
            return { ...newItem, unit_price: existing.unit_price, total: existing.total }
        }
        return newItem
      })
      return {
        ...
        total: itemsWithPrices.reduce((s, i) => s + i.total, 0),
      }
    }
...
function compareVersions(
  ...
  addedItems: string[]
  removedItems: string[]
  priceChanges: Array<{ item: string; oldPrice: number; newPrice: number }>
} {
...
describe('報價 ↔ 行程同步', () => {
  ...
  describe('版本管理', () => {
    ...
    it('比較版本差異 — 價格變更', () => {
      ...
      expect(diff.priceChanges[0]).toEqual({
        item: '清邁文華酒店',
        oldPrice: 12000,
        newPrice: 20000,
      })
