...
handleDeleteTour = async (tour) => {
  ...
  await supabase.from('itineraries').update({ tour_id: null, tour_code: null }).eq('tour_id', tour.id)
...
const ORDERS_LIST_FIELDS = [
  'id', 'order_number', 'tour_id', 'tour_name',
  'contact_person', 'sales_person', 'assistant',
  'payment_status', 'paid_amount', 'remaining_amount',
  'member_count', 'code', 'created_at',
].join(',')
