...
| # | 項目 | 說明 | 預估 |
|---|------|------|------|
| 1 | **Online V2 功能入口隱藏** | 社群分享(posts)、訊息中心(conversations)、旅伴名單(friends) 的 API 引用不存在的表，點擊會 400 錯誤 | 1h |
| 2 | **Online Profile 顯示名稱** | profiles.display_name 為空，顯示 email prefix 而非真實名字 | 30m |
| 3 | **itinerary_days / itinerary_items RLS Policy** | 這兩張是行程核心表，目前 RLS 啟用但無 Policy → 前端直接操作會被拒絕（ERP 若全走 service role 則不影響，需確認） | 1h |
| 4 | **journal_lines RLS Policy** | 會計分錄核心表，同上需確認是否前端直接存取 | 30m |
