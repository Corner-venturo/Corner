...
| 檔案 | 逃逸數 | 優先級 | 狀態 |
|------|--------|--------|------|
| `workspace-store.ts` | 15 | P0 | ⏳ 待處理 |
| `lib/db/index.ts` | 7 | P0 | ⏳ 待處理 |
| `lib/supabase/api.ts` | 6 | P0 | ⏳ 待處理 |
...
5. **`src/lib/supabase/api.ts`** (6 處)
   - 問題：Supabase API 回傳值型別
   - 方案：使用 Supabase 的 `Database` 型別定義

6. **`src/core/services/base.service.ts`** (5 處)
   - 問題：基礎 Service 泛型不足
   - 方案：完善 Service 泛型約束
