...
function isSafeToModify(filePath) {
  return (
    !filePath.includes('node_modules') && !filePath.includes('.git') && filePath.startsWith('src/')
  )
}
...
function fixTypeEscapes(dryRun = false) {
  ...
  try {
    const output = execSync(`rg "as unknown" src/ -l 2>/dev/null || grep -rl "as unknown" src/`, {
      encoding: 'utf-8',
    })
