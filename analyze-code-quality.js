...
function analyzeMemoryLeaks() {
  ...
  try {
    ...
    const addLines = addListenerOutput.trim().split('\n').filter(line => line);
      ...
      if (match) {
        ...
        try {
          ...
          if (!fileContent.includes('removeEventListener')) {
            results.memoryLeaks.push({ file, lineNum: parseInt(lineNum), type: 'missing-remove-listener' });
...
function generateReport() {
  ...
  console.log(`  🔓 型別逃逸 (as unknown): ${results.typeEscapes.reduce((sum, f) => sum + f.count, 0)} 處 (${results.typeEscapes.length} 個檔案)`);
  console.log(`  ⏰ setTimeout 使用: ${results.setTimeoutUsage.length} 處`);
  console.log(`  💾 潛在記憶體洩漏: ${results.memoryLeaks.length} 處`);
  console.log(`  📝 TODO/FIXME: ${results.todos.reduce((sum, f) => sum + f.count, 0)} 處 (${results.todos.length} 個檔案)`);
